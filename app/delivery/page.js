...
const Modal = ({ title, onClose, children, wide, extra }) => (
  <div onClick={e => e.target === e.currentTarget && onClose()}
    style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.8)', display:'flex', alignItems:'flex-start', justifyContent:'center', zIndex:1000, padding:12, paddingTop:20, backdropFilter:'blur(4px)', animation:'fadeIn .2s ease' }}>
...
function useData() {
  ...
  useEffect(() => {
    // const t = setInterval(fetchAll, 6000000)    return () => clearInterval(t)
  }, [fetchAll])
...
function Orders({ data, refetch, user }) {
    ...
    <div className="page-enter">
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:12, background:'rgba(255,255,255,.03)', padding:'10px 14px', borderRadius:12, border:'1px solid rgba(255,255,255,.06)' }}>
        ...
        <select value={fZ} onChange={e=>setFZ(e.target.value)} style={{ padding:'6px 10px', borderRadius:8, border:'1px solid rgba(255,255,255,.12)', fontSize:12, fontFamily:'inherit', direction:'rtl', background:'#0d1018', color:'white' }}>
          <option value=''>كل المناطق</option>{data.zones.map(z=><option key={z.id} value={z.name}>{z.name}</option>)}
        </select>
  ...
  <div style={{ display:'flex', gap:4 }}>
    <Btn onClick={() => setExpId(isExp?null:o.id)} small color="rgba(59,91,254,.4)">عرض</Btn>
    <Btn onClick={() => setInvoice(o)} small color="#10b981">فاتورة</Btn>
    {can(user,'orders_w') && <Btn onClick={() => setModal(o)} small color="#6b7280">تعديل</Btn>}
    {can(user,'orders_w') && <Btn onClick={() => setConf({ msg:'حذف الطلب رقم ' + o.id, ok:() => deleteOrder(o.id) })} small color="#ef4444">حذف</Btn>}
  </div>
</Td>
              </Tr>,
              isExp && (
    ...
    <div className="page-enter">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
        ...
        <Card>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            <DonutChart data={[
              { v:delivered.length, color:'#10b981' },
              { v:returned.length,  color:'#f59e0b' },
              { v:cancelled.length, color:'#ef4444' },
              { v:orders.filter(o=>!['تم التسليم','مرتجع','ملغي'].includes(o.status)).length, color:'#3b5bfe' }
            ]} size={90}/>
            <div style={{ flex:1 }}>
